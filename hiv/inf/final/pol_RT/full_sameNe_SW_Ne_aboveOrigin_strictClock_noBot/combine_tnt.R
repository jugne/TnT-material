...
wd <- "~/Documents/Source/TnT-material/hiv/inf/final/pol_RT/full_sameNe_SW_Ne_aboveOrigin_strictClock_noBot/combined"

dir.create(wd)
setwd(wd)
burnIn <- 20
logs <- list.files(path=paste0("../"), pattern="env_pol_saConstrained_s1_sameNe_noBot_run[0-9]*\\.log$", full.names = TRUE)
trees<- list.files(path=paste0("../"), pattern="env_pol_saConstrained_s1_sameNe_noBot_run[0-9]*\\.transmission.trees$", full.names = TRUE)
...
system(paste("java -jar",transmission_analyser_path,"-tree env_pol_saConstrained_s1_sameNe_noBot_combined.transmission.trees",
             "-out env_pol_saConstrained_s1_sameNe_noBot_combined.transmission.log"))
